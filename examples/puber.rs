...
fn handle_comand(cmd: &str, socket: &mut Socket) {
    ...
    let big: usize = 6/* * 1024 * 1024*/; // this to force a partial write
...
fn main() {
    ...
    let mut socket = session.create_socket(SocketType::Pub).unwrap();

    socket.bind("tcp://127.0.0.1:5459").unwrap();
