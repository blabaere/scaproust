...
fn before_each() -> (Session, Socket, Socket, Socket, Socket) {
    let _ = ::env_logger::init();
    let mut session = make_session();
    let publ = session.create_socket::<Pub>().expect("Failed to create socket !");
    ...
    (session, publ, sub1, sub2, sub3)
}

#[test]
fn broadcast_a_message_through_local_endpoint() {
    let (session, mut publ, mut sub1, mut sub2, mut sub3) = before_each();
...
#[test]
fn ignores_messages_based_on_subscriptions() {
    let (session, mut publ, mut sub1, mut sub2, mut sub3) = before_each();
    let url = urls::tcp::get();
